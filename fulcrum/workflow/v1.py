"""
`fulcrum.workflows.v1`: Basic ID and quantification workflow, employing a single-pass search with downstream processing
steps for PSM and precursor/peptide confidence estimation, as well as protein inference and quantification.

This workflow is intended for general use in proteomics analyses is extremely flexible, permitting a wide variety of
uses depending on the selected backend implementations and their configuration.
"""
...
def v1_workflow(
    ...
    To use, call ``fulcrum.fulcrum(workflow="v1")`` or run ``fulcrum`` using the following TOML:
    ...
    search : dict, optional
        ...
        Include a ``backend`` from the :py:mod:`fulcrum.search` registry (default: :py:func:`~fulcrum.search.read_existing`)
    ...
    peptide_quant : dict, optional,
        ...
        Include a ``backend`` from the :py:mod:`fulcrum.quant.peptide` registry (default: :py:func:`~fulcrum.quant.peptide.basic`)
        or a suitable ``callable``.
    ...
    protein_rollup : dict, optional
        ...
        Include a ``backend`` from the :py:mod:`fulcrum.quant.protein` registry (default: :py:func:`~fulcrum.quant.protein.basic`)
        or a suitable ``callable``.
    ...
    output : dict, optional
        ...
        This is convenient for setting a single ``backend`` or :py:func:`filtering options <fulcrum.output.util.filter_psms>`.
    ...
    peptide_output : str | dict, optional
        ...
        * ``backend``: The backend that will write peptide-level results.
          ...
          Either a string referring to a backend or plugin from :py:mod:`fulcrum.output` registry, or a ``Callable``. If a
          ...
          Default: :py:func:`~fulcrum.output.basic.write_parquet`
    ...
    protein_output : str | dict, optional
        ...
        * ``backend``: The backend that will write protein-level results
          Either a string referring to a backend or plugin from the :py:mod:`fulcrum.output` registry, or a ``Callable``.
