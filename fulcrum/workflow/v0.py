"""
`fulcrum.workflows.v0`: Initial basic workflow, without protein inference or quantification. This workflow is primarily
intended for use in validating PSM and/or precursor/peptide identifications, or for creating spectral libraries.
"""
...
def v0_workflow(
    ...
    To use, call ``fulcrum.fulcrum(workflow="v0")`` or run ``fulcrum`` using the following TOML:
    ...
    search : dict, optional
        ...
        * ``backend``: The backend that will compute or read search results.
          ...
          Default: :py:func:`~fulcrum.search.read_existing`
    ...
    output : str|dict, optional
        ...
        * ``backend``: The backend that will compute or read search results.
          ...
          Default: :py:func:`~fulcrum.output.write_csv`
